...
function initWidget() {
    ...
    var onIVSubmit = function() {
        khann.evalIV("alphanum", getCanvasAsIV(), onIVResponse);
    };
    ...
    var onOVSubmit = function() {

        // Get user submitted IV classification as an OV
        var bit = parseInt($("#classification").val());
        var ov = Array(36);
        for (var i = 0; i < 36; i++)
            ov[i] = bit == i ? 1 : 0;

        khann.submitIV("alphanum", getCanvasAsIV(), ov);
    };
    ...
    khann.registerCallbacks(onResetWidget, onOVSubmit);
